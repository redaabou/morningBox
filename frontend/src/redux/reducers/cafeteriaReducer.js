const initialState = {
  ...
  switch (action.type) {
    ...
    case 'DELETE_CAFETERIA_REQUEST':
    return { ...state, loading: true, error: null };
    case 'DELETE_CAFETERIA_SUCCESS':
    return { ...state, loading: false, cafeterias: state.cafeterias.filter((cafeteria) => cafeteria._id !== action.payload) };
    case 'DELETE_CAFETERIA_FAIL':
    return { ...state, loading: false, error: action.payload};

    case 'UPDATE_CAFETERIA_REQUEST':
    return { ...state, loading: true, error: null };
    case 'UPDATE_CAFETERIA_SUCCESS':
    return { ...state, loading: false, cafeterias: state.cafeterias.map((cafeteria) => cafeteria._id === action.payload._id ? action.payload : cafeteria) };
    case 'UPDATE_CAFETERIA_FAIL':
    return { ...state, loading: false, error: action.payload};
